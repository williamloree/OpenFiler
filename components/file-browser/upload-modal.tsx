...
export function UploadModal({ onClose, onUpload, uploading, uploadProgress, uploadStatus }: UploadModalProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [dragOver, setDragOver] = useState(false);

  return (
    <div className="fb-modal-overlay" onClick={() => !uploading && onClose()}>
      <div className="fb-modal" onClick={(e) => e.stopPropagation()}>
        <div className="fb-modal-header">
          <h3>Upload de fichiers</h3>
          <button className="fb-modal-close" onClick={() => !uploading && onClose()}>
            &times;
          </button>
        </div>
